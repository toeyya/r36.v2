...
function mmmap_client_init() {
	...
	mmmap = new MMMap(mmmap_div,18.99461,99.71191,11,'political');
  mmmap.hideScale();
...
function drawObjectSet(set) {
  ...
  var area = [
				...
				foreach ($S_amphur_js->result_array() as $row)
				{
				
						$amp_name = $row['name'];
						$amp_code = $row['s_value'];
						$amp_color = $row['risk_color'];

						
			    		echo "[['".$amp_name."'],['".$amp_code."'],['".$amp_color."']],";
...
$('ul.tabs').each(function(){
  ...
  $('#container').highcharts({
        ...
        series: [{
            type: 'pie',
            name: 'อัตราการสัมผัสโรค ของแต่ละอำเภอ',
            data: [
            
			<?php 
			
				foreach ($S_amphur_pie->result_array() as $row)
				{
						...
						echo "{name: '".$amp_name."',y: ".$row['no_ppe'].",color :'".$row['risk_color']."'},";
...
<div style="display:block; width:95%; border:1; border-style:solid; border-color:#F00; padding:5px 5px 5px 5px; margin-top:10px; margin-bottom:10px; background-color:#FFD7D7; text-align:center; font-weight:bold; font-size:16px; color:#F00;">
 ...
 > <a href="<?php echo site_url($sector_link); ?>" target="_self">ภาค</a> > <a href="<?php echo site_url($province_link); ?>" target="_self">จังหวัด</a>
...
foreach ($S_province->result_array() as $row)
{
	...
	//$tbAmphur = $this->db->query("SELECT * FROM n_amphur where province_id=".$pro_id);
 	foreach ($S_amphur_list->result_array() as $row2)
	{
		//echo "<li>";
		$amp_id = $row2['id_province'].$row2['id_amphur'];
		$amp_name = $row2['name'];
		$province_link = "gis_map/map_amphur/".$map_id."/".$row2['id_province']."/".$row2['id_amphur']."";
		
		echo "
	<input type='button' onClick=\"window.location='".site_url($province_link)."'\" value='".$amp_name."'>";
...
<table width="100%" border="0" cellspacing="2" cellpadding="2">
  <tr>
    <td width="25%" align="left" valign="top" bgcolor="#99FF66"><span id="font_header">อำเภอ (amphur) </span></td>
...
<?php 
	...
	//$tbAmphur_list = $this->db->query("SELECT * FROM n_amphur where province_id=".$pro_id);
 	...
 	foreach ($S_amphur_data->result_array() as $row2)
	{
 ...
 ?>
  ...
  <tr>
    <td align="left" valign="top" bgcolor="#FFFFFF"><a href="<?php echo site_url($province_link); ?>"><?php echo $amp_name; ?></a> </td>
