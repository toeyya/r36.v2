  ...
  function login()
   {
	...
	{		
		$id=clean_url($id);
		$result=$this->db->GetRow("SELECT uid,confirm_admin,confirm_province FROM n_user WHERE uid= ? and gen_id = ? ",array($id,$c));
		$data['result'] = $result;
		if($data['result']['uid']){
	...
	{	// ผู้สัมผัสโรคไม่เช็คการซ้ำกัน
		...
		// ผู้ใช้ระบบ
		if(empty($_GET['uid'])){
			$dup = $this->db->GetOne("select idcard from n_user where idcard = '".$_GET['idcard']."'");	
			echo ($dup1 || $dup)? "false":"true";
			
		}else{
			$dup = $this->db->GetOne("select idcard from n_user where idcard = '".$_GET['idcard']."' and uid <> ? ",$_GET['uid']);					
			echo ($dup1 || $dup)? "false":"true";
		}		
	}
